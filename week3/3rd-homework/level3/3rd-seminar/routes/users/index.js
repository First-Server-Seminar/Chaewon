...
router.post('/signup', (req, res) => {
    ...
    crypto.randomBytes(64, (err, buf) => {
        ...
        crypto.pbkdf2(password, salt, 100000, 64, 'sha512', (err, key) => {
            const sortedPassword = key.toString('base64');
            usersDB.push({
                id: id,
...
router.post('/signin', (req, res) => {
    ...
    const userSort = user.salt;
    const userPassword = user.password;

    crypto.pbkdf2(password, userSort, 100000, 64, 'sha512', (err, key) => {
        const sortedPassword = key.toString('base64');
        if (sortedPassword == userPassword) {
            console.log('로그인 성공.');
